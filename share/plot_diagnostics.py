...
# dependencies = ["numpy", "matplotlib", "typing", "PyQt6"]
# ///
...
def add_linspace(lines: Iterator[str], style):
    ...
    x = np.linspace(
        float(linspace[0]), float(linspace[1]), length, endpoint=bool(linspace[2])
    )
...
def add_scatter_shifts_final():
    ...
    for label, x, y in zip(
        add_scatter_shifts.labels, add_scatter_shifts.xs, add_scatter_shifts.ys
    ):
    ...
    plt.plot(
        [x for x in add_scatter_shifts.xs],
        [y for y in add_scatter_shifts.ys],
        color="black",
        linestyle="-",
        linewidth=1,
    )
    ...
    for jj, filename in enumerate(filenames):
        ...
        with open(filename) as file:
            ...
            if not all(key in expected_keys for key in header.keys()):
                raise ValueError(
                    f"Invalid file header: keys={header.keys()}, expected_keys={expected_keys}"
                )
