...
namespace qn::ctf {
    ...
    auto Patches::from_stack(
        ...
        // auto& stream = Stream::current(options.device);
        // f64 t0{}, t1{}, t2{}, t3{}, t4{};
        for (const auto& image_metadata: metadata_sorted) {
            ...
            // stop.record(stream);
            // stop.synchronize();
            // t0 += noa::Event::elapsed(start, stop).count();
            ...
            // stop.record(stream);
            // stop.synchronize();
            // t1 += noa::Event::elapsed(start, stop).count();
            ...
            // stop.record(stream);
            // stop.synchronize();
            // t2 += noa::Event::elapsed(start, stop).count();
            ...
            // stop.record(stream);
            // stop.synchronize();
            // t3 += noa::Event::elapsed(start, stop).count();
            ...
            // stop.record(stream);
            // stop.synchronize();
            // t4 += noa::Event::elapsed(start, stop).count();
        ...
        // auto tc = static_cast<f64>(metadata_sorted.size());
        // Logger::trace("t0={}, t1={}, t2={}, t3={}, t4={}", t0/tc, t1 / tc, t2/tc, t3/tc, t4/tc);
